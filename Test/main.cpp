...
//„тобы без заморочек ссылатьс€ на наше окошко
...
int main(int argc, char *argv[])
{
    ...
    quint16 port=0;
    QProcess me;//когда кончитс€ область видимости - тогда и прога канет всуе


    if(!myarg.contains("-child")){
        //ћы родитель или сервер
        //ќткрывает сокет, запускаем ребенка
        QStringList argums;
        argums << "-child";
        ...
        //— этим ключем мы не запускаем ребенка
    ...
    //пишем порт, который слушаем на форму, чтобы знать как подключатьс€
    w.writeLog("INIT",QString("Port:%1").arg(tcpServer.serverPort()));

    //воссоздаем семью из ребенка безпризорника и родител€ партеногенезника
