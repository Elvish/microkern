...
// ƒалее следует отладочные переменные, которые никому никогда не нужны
    ...
    //#define DEBUG_OTLADKA    //дл€ синхронизации с отсыльщиком пакетов
...
// выше были отладочные переменные, которые никому никогда не нужны и не вли€ют на ход выполнени€ программы
...
//Ќеобходимо вызвать в качестве инициализации буфера - возвращает true если все параметры в норме и false если нет
...
//—пособы расчета CRC... ≈сть разные способы, кроме оригинального CRC8 :)
//Ёти методы были протестированны на способность перехвата типичных ошибок канального уровн€
//лучшим оказалс€ "mode 19"
...
static void TestPacket(void)
{
	...
	Len = PrePtr - PreBuffer;			//ƒлина пакета вместе с заголовком
	#ifdef DEBUG2
	...
	if(BufLen - (EndPtr - NormalBuffer) < Len + 2 + 1 + 4){	//Ќет места до конца буфера 4-метка рецикла буфера
...
//ƒолжна вызыватьс€ при приеме байта где-то извне
void BP_ReceiveChar(unsigned char c)
{
	...
	//–ежим приема заголовка и данных
	if(c == BINPROT_EXT){// онец пакета
		...
		//ѕакет прин€т, провер€ем CRC, SERIAL и т.п.
...
//копирует пакет в место назначени€, и удал€ет его из очереди
...
void BP_ClearBuffer(void)	//занул€ет буфер
{
	BeginPtr = EndPtr;
}
